...
def training_pipeline(model_name, text_col, max_length, batch_size, num_epochs, metric_name, random_state, learning_rate):
    # Data loading
    
    df_main = pd.read_csv('../data/silver_data/silver_labels_gpt_3.5_turbo.csv')
    df_train, df_val = split_dataset(df_main, id_col='id', stratify_col='silver_label', test_size=0.1, random_state=random_state)

    df_train['silver_label'].value_counts()
    df_train['depression'] = df_train['silver_label'].apply(lambda x: True if x in ['Depression', 'Comorbid (Depression + Anxiety)'] else False)
    df_train['anxiety'] = df_train['silver_label'].apply(lambda x: True if x in ['Anxiety', 'Comorbid (Depression + Anxiety)'] else False)

    df_val['silver_label'].value_counts()
    df_val['depression'] = df_val['silver_label'].apply(lambda x: True if x in ['Depression', 'Comorbid (Depression + Anxiety)'] else False)
    df_val['anxiety'] = df_val['silver_label'].apply(lambda x: True if x in ['Anxiety', 'Comorbid (Depression + Anxiety)'] else False)
    ...
    labels = [label for label in test_dataset.features.keys() if label not in ['id', 'text']]
    ...
    train_dataset_encoded = train_dataset.map(preprocess_data, batched=True, remove_columns=train_dataset.column_names)
    val_dataset_encoded = val_dataset.map(preprocess_data, batched=True, remove_columns=val_dataset.column_names)
    test_dataset_encoded = test_dataset.map(preprocess_data, batched=False, remove_columns=test_dataset.column_names)
    ...
    predictions_raw, predictions_proc = inference_pipeline(texts, model, tokenizer, max_length, id2label)
...
def main():
    ...
    training_pipeline(args.model_name, args.text_col, args.max_length, args.batch_size, args.num_epochs, args.metric_name, args.random_state, args.learning_rate)
