...
def load_and_preprocess_dataset(random_state, model_name, text_col, label_col, max_length):    
    # 1. Load the given csv dataset
    df_silver_labels = pd.read_csv('../data/silver_data/silver_labels_gpt_3.5_turbo.csv')
    ...
    train_dataset = _tokenize_and_convert(df_train, model_name, text_col, label_col, max_length)
    val_dataset = _tokenize_and_convert(df_val, model_name, text_col, label_col, max_length)
...
def _tokenize_and_convert(df, model_name, text_col, label_col, max_length):
    ...
    tokenized_inputs = tokenizer(df[text_col].tolist(), truncation=True, padding="max_length", max_length=max_length)
...
def inference_pipeline(texts, model, tokenizer, max_length):
    ...
    for i in tqdm(
        ...
        tokenized_inputs = tokenizer(batch_inputs, truncation=True, padding="max_length", max_length=max_length, return_tensors='pt')
...
def training_pipeline(model_name, label_col, text_col, max_length, batch_size, num_epochs, metric_name, random_state, learning_rate):
    train_dataset, val_dataset = load_and_preprocess_dataset(random_state, model_name, text_col, label_col, max_length) 
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Text Classification Training Pipeline')
    ...
    free_memory()
    training_pipeline(args.model_name, args.label_col, args.text_col, args.max_length, args.batch_size, args.num_epochs, args.metric_name, args.random_state, args.learning_rate)
    free_memory()
